...
**/
void push_substitution_sub_store(substitution_store* store, const substitution* new_sub){
  unsigned int sub_no = alloc_store_substitution(store);
  copy_substitution_struct(get_substitution(sub_no, store), new_sub, store->ssi);
}
